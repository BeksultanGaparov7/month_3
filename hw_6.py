...
class UserManager:
    ...
    def add_user(self, name, age, email=None, job=None):
        sql = "INSERT INTO users (name, age, email, job) VALUES (?, ?, ?, ?)"
        conn = self._connect()
        try:
            cur = conn.cursor()
            cur.execute(sql, (name, age, email, job))
            conn.commit()
    ...
    def update_user(self, user_id, name=None, age=None, email=None, job=None):
        fields = []
        params = []
        if name is not None:
            fields.append("name = ?")
...
def parse_args():
    ...
    p.add_argument('--job', help='Должность')
    ...
    p.add_argument('--job', help='Новая должность')
...
def main():
    ...
    if args.cmd == 'add':
        uid = db.add_user(args.name, args.age, args.email, args.job)
        print(f'✅ Пользователь создан с ID={uid}')

    elif args.cmd == 'list':
        rows = db.get_all_users()
        print("ID | Name       | Age | Email               | Job")
        print("------------------------------------------------------------")
        for r in rows:
            print(f"{r['id']:2d} | {r['name']:<10} | {r['age'] or '-':>3} | {r['email'] or '-':<19} | {r['job'] or '-'}")

    elif args.cmd == 'update':
        count = db.update_user(args.id, args.name, args.age, args.email, args.job)
        if count:
